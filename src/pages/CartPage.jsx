    ...
    <div style={{ minHeight: '70vh', background: 'var(--base)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="empty">
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
          ...
          <button className="btn btn-s btn-lg" onClick={() => navigate('/digitals')}>Digital Store →</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: '100vh', background: 'var(--base)', paddingBottom: 80 }}>
      <div style={{ padding: '48px 48px 28px', borderBottom: '1px solid var(--border)' }}>
        <div className="wrap">
          ...
          <div className="back" onClick={() => navigate('/shop')}>← Continue Shopping</div>
            ...
            {cart.map(item => (
              <div key={item.id} className="card" style={{ display: 'flex', gap: 18, marginBottom: 12, padding: 18, animation: 'fadeUp .3s ease' }}>
                <div style={{ width: 96, height: 96, borderRadius: 'var(--r)', overflow: 'hidden', flexShrink: 0, background: 'var(--glass)', cursor: 'pointer' }}
                  ...
                  {item.image ? <img src={item.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', background: '#1B4332', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, opacity: .3 }}>🎨</div>}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {item.charityName && <div style={{ fontSize: 11, color: 'var(--muted)', display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ color: 'var(--sage)' }}>🌿</span> Supporting: {item.charityName}</div>}
                </div>
          ...
          <div style={{ position: 'sticky', top: 90 }}>
            <div className="card" style={{ padding: 24 }}>
              ...
              <div style={{ marginTop: 18, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
                ...
                {discount && <div style={{ fontSize: 11, color: 'var(--sage)', marginTop: 4 }}>✓ {discount.code} — saving £{discount.discount.toFixed(2)}</div>}
                <div style={{ fontSize: 10, color: 'var(--subtle)', marginTop: 4 }}>Try: IMPACT10, WELCOME15, ARTFREE</div>
              </div>

              <button className="btn btn-p btn-lg" style={{ width: '100%', justifyContent: 'center', marginTop: 18 }} onClick={proceedToCheckout}>
                Proceed to Checkout →
              </button>

              {/* Trust badges */}
              <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 14, fontSize: 11, color: 'var(--muted)' }}>
                <span>🔒 Secure</span><span>💳 Stripe</span><span>🏛 Certified</span>
              </div>
            </div>

            {/* Impact summary */}
            <div className="card" style={{ padding: 18, marginTop: 12, background: 'rgba(23,124,29,.04)', border: '1px solid rgba(23,124,29,.12)' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--accent)', marginBottom: 8 }}>🌿 Your Impact</div>
