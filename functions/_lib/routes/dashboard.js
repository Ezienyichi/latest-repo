...
dashboard.get('/artist', authenticate, requireRole('ARTIST'), async (c) => {
  ...
  try {
    const profile = await prisma.artistProfile.findUnique({ where: { userId: c.get('userId') } });
    if (!profile) return c.json({ error: 'No artist profile' }, 404);

    const [productsList, orderItems, totalEarnings, partnerships] = await Promise.all([
      ...
      prisma.artistCharityPartnership.findMany({ where: { artistId: profile.id }, include: { charity: { select: { id: true, name: true, logo: true, sdgIds: true } } } }),
    ]);
    ...
    return c.json({
      ...
      stats: {
        ...
        activeListings: productsList.filter(p => p.status === 'ACTIVE').length,
        draftListings: productsList.filter(p => p.status === 'DRAFT').length,
...
dashboard.patch('/artist/profile', authenticate, requireRole('ARTIST'), async (c) => {
  const prisma = c.get('prisma');
  try {
    const profile = await prisma.artistProfile.findUnique({ where: { userId: c.get('userId') } });
    if (!profile) return c.json({ error: 'No profile' }, 404);
    const { displayName, artistStatement, biography, location, socialLinks, sdgIds, avatarUrl, exhibitions, awards } = await c.req.json();
  ...
  } catch (e) { console.error(e); return c.json({ error: 'Update failed' }, 500); }
});

// ── CHARITY DASHBOARD ───────────────────────────────────────
dashboard.get('/charity', authenticate, requireRole('CHARITY'), async (c) => {
  const prisma = c.get('prisma');
  try {
...
dashboard.patch('/charity/profile', authenticate, requireRole('CHARITY'), async (c) => {
  const prisma = c.get('prisma');
  try {
    const profile = await prisma.charityProfile.findUnique({ where: { userId: c.get('userId') } });
    if (!profile) return c.json({ error: 'No profile' }, 404);
    const { name, mission, registrationNo, sdgIds, websiteUrl, target, logo } = await c.req.json();
...
dashboard.post('/charity/messages', authenticate, requireRole('CHARITY'), async (c) => {
  const prisma = c.get('prisma');
  try {
    const profile = await prisma.charityProfile.findUnique({ where: { userId: c.get('userId') } });
    if (!profile) return c.json({ error: 'No profile' }, 404);
    const { subject, bodyHtml, messageType } = await c.req.json();
    if (!subject || !bodyHtml) return c.json({ error: 'Subject and body required' }, 400);
    const msg = await prisma.charityMessage.create({ data: { charityId: profile.id, messageType: messageType || 'APPRECIATION', subject, bodyHtml, recipientCount: profile.funderCount } });
    return c.json(msg, 201);
  } catch (e) { return c.json({ error: 'Send failed' }, 500); }
});

dashboard.post('/charity/templates', authenticate, requireRole('CHARITY'), async (c) => {
  const prisma = c.get('prisma');
  try {
    const profile = await prisma.charityProfile.findUnique({ where: { userId: c.get('userId') } });
    if (!profile) return c.json({ error: 'No profile' }, 404);
    const { name, subject, bodyHtml } = await c.req.json();
    if (!name || !subject) return c.json({ error: 'Name and subject required' }, 400);
...
dashboard.delete('/charity/templates/:id', authenticate, requireRole('CHARITY'), async (c) => {
  const prisma = c.get('prisma');
  try {
    await prisma.messageTemplate.delete({ where: { id: c.req.param('id') } });
    return c.json({ message: 'Deleted' });
  } catch (e) { return c.json({ error: 'Delete failed' }, 500); }
});

dashboard.post('/charity/resources', authenticate, requireRole('CHARITY'), async (c) => {
  const prisma = c.get('prisma');
  try {
    const profile = await prisma.charityProfile.findUnique({ where: { userId: c.get('userId') } });
    if (!profile) return c.json({ error: 'No profile' }, 404);
    const { title, fileUrl, fileType, visibility } = await c.req.json();
    if (!title || !fileUrl) return c.json({ error: 'Title and file URL required' }, 400);
...
dashboard.delete('/charity/resources/:id', authenticate, requireRole('CHARITY'), async (c) => {
  const prisma = c.get('prisma');
  try {
...
// ── ADMIN DASHBOARD ─────────────────────────────────────────
dashboard.get('/admin', authenticate, requireRole('ADMIN'), async (c) => {
  const prisma = c.get('prisma');
  try {
